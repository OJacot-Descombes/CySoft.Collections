...
{
    private readonly Func<T, double[][]> _accessMultiplePoints;
    private readonly Func<T, double[]> _accessSinglePoint;
    ...
    public void Add(T value)
    {
        if (_accessSinglePoint != null) {
            ...
            double[][] points = _accessMultiplePoints(value);
    ...
    public NearestNeighborEnumerable<T> NearestNeighbors(int maxPointsReturned, params double[] searchPoint)
    {
        return new NearestNeighborEnumerable<T>(this, searchPoint, maxPointsReturned, SquareEuclideanDistance.Instance);
    }

    public NearestNeighborEnumerable<T> NearestNeighbors(DistanceFunction distanceFunction, int maxPointsReturned, params double[] searchPoint)
    {
        return new NearestNeighborEnumerable<T>(this, searchPoint, maxPointsReturned, distanceFunction);
    }

    public MaxHeap<double, KDEntry<T>> FindNearestNeighbors(DistanceFunction distanceFunction, int maxPointsReturned, params double[] searchPoint)
    {
        var pendingPaths = new MinHeap<double, KDNode<T>>();
        var evaluatedPoints = new MaxHeap<double, KDEntry<T>>();
        int pointsRemaining = Math.Min(maxPointsReturned, _count);
    ...
    public bool Remove(KDEntry<T> entry)
    {
        ...
        for (int i = 0; i < cursor._count; i++) {
            if (cursor._leafEntries[i].Equals(entry)) {
                ...
                cursor._leafEntries[cursor._count - 1] = default;
                do {
                    cursor._count--;
